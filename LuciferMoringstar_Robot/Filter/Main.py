...
If you do not receive the movie even after entering the correct name.
@admin type movie name Inform the admin in this format.
We will upload within 24 hours as we can.
...
ശരിയായ പേര് നൽകിയിട്ടും നിങ്ങൾക്ക് സിനിമ ലഭിക്കുന്നില്ലെങ്കിൽ.
@admin ടൈപ്പ് മൂവി നാമം ഈ ഫോർമാറ്റിൽ അഡ്മിനെ അറിയിക്കുക.
ഞങ്ങൾ 24 മണിക്കൂറിനുള്ളിൽ അപ്‌ലോഡ് ചെയ്യാൻ ശ്രമിക്കും""",
              parse_mode="html",
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            buttons = [
                [
                    InlineKeyboardButton('Dev 🤦', url=f'https://t.me/paulwalkerassitbot')
                ]
                ]
            ...
            for files in filedetails:
                ...
                buttons = [
                    [
                        InlineKeyboardButton('❤️ Join ❤️', url=f'https://t.me/fbm_cinema')
                    ]
                    ]
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                await query.answer("If your are smart i am your dad 😒",show_alert=True)
                return
            ident, file_id = query.data.split("#")
            filedetails = await get_file_details(file_id)
            for files in filedetails:
                ...
                buttons = [
                    [
                        InlineKeyboardButton('🤔 Support 🤔', url=f'https://t.me/hf_links')
                    ]
                    ]
        ...
        await query.answer("മിസ്റ്റർ ഇത് തനിക്ക് ഉള്ളത് അല്ല 🤭",show_alert=True)
